...
Parse only the token directly after the command, case-insensitive:

| Token after command | Mode |
|---|---|
| nothing / anything else | One-time consult (default) |
| `true`, `-chk`, `-checkpoints` | Session checkpoints |

- `true` elsewhere in the message, or another command's argument, keeps the default.
- Strip a flag from the request before interpreting the rest.
- One-time: exactly one consult now on the current request (or the attached question); read it, act as executor; consult again only if the user invokes the skill again. Checkpoint rules do not apply.
...
- No flag: "One-time Fable 5.1 consult (`high` effort via synchronous `Agent(subagent_type: "claude-advisor")`, no skill-imposed timeout); session checkpoints off." Run the single consult, continue as executor.
- Flag (`true` / `-chk` / `-checkpoints`): "Advisor checkpoints active for this session (Fable 5.1 at real `high` reasoning effort via synchronous `Agent(subagent_type: "claude-advisor")`, no skill-imposed wall-clock timeout, executor = current session model)." Continue the user's request, consulting per "Checkpoint mode" and [references/checkpoints.md](references/checkpoints.md).

Check before acting: trigger matched, mode from the very next token, transport verified, advisor = Fable 5.1 at `high`.
