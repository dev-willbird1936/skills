...
3. Run `/compress maximum` on the restructured result, not on the lossy output. This is the compact variant, for injection into context by a hook. Keep its loss list.
4. Validate both against the original intent: every rule, bound, exception and switch still fires once, in the right place; nothing new added. The compact variant may drop examples and rationale, never a switch, bound, exception or name the consumer must match.
